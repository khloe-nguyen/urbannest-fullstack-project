...
import { CreateNewEmployeeRequest } from "./api/createEmployeeApi";
...
export default function CreateEmployee() {
  ...
  const createNewEmployee = CreateNewEmployeeRequest();
  ...
  const onCreateNewEmployee = (ev) => {
    ...
    if (isOk) {
      ...
      createNewEmployee.mutate(formData, {
        onSuccess: (response) => {
  ...
  if (createNewEmployee.isPending) {
    return <WaitingPopUp />;
  }
      ...
      <Container>
        <Left>
          ...
          {option == "info" && (
            <>
              <div>
                <label>Email</label>
                <TextInput state={email} setState={setEmail} placeholder={"abcd@gmail.com"} />
              </div>
              ...
              {errors.confirm && <h5>{errors.confirm}</h5>}
              <div>
        ...
        <Right>
          <div>
            <label>Avatar *</label>
            {image ? (
              <Avatar size="200" round src={URL.createObjectURL(image)} />
            ) : (
