...
export const UserRefillRequest = () => {
  const request = async () => {
    const response = await axiosClient.get("http://localhost:8080/userCM/userRefill");
...
export default function UserProfile() {
  ...
  return (
    <>
          ...
          <div className="avatar">
            <div className="left">
              ...
              <div style={{ fontWeight: "bolder", fontSize: "24px" }}>
                {user?.data?.data?.preferredName != null && user?.data?.data?.preferredName != ""
            ...
            <div style={{ display: "flex", flexDirection: "column", margin: "6%" }}>
              ...
              {userRefill?.data?.data?.complete ? (
                ...
                <>
                  ...
                  <div>Before you book or host on Airbnb, you’ll need to complete this step.</div>
                  <button
      ...
      <Footer percent={"72%"} />
      {isSuccess && (
        <SuccessPopUp action={() => setIsSuccess()} message={"Please wait for admin to response"} />
      )}
    </>
  );
}
